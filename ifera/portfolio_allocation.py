...
def find_optimal_capital_allocation_from_csv(
    ...
    Args:
        ...
        dtype: Output allocation dtype. Defaults to the float64 input-series dtype.
    ...
    strategy_frames = tuple(_load_strategy_csv(path) for path in csv_paths)
...
def _load_strategy_csv(path: Path) -> pl.DataFrame:
    ...
    try:
        strategy_frame = raw_frame.select(
            ...
            (pl.col(return_column).cast(pl.Float64, strict=True) / 100.0).alias(
                "return"
            ),
        )
...
def _plot_equity_curve(portfolio_name: str, daily_results: pl.DataFrame) -> None:
    ...
    axis.set_yscale("log")
    axis.set_title(f"Optimized equity curve: {portfolio_name}")
