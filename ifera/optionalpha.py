...
class SplitGenerator:
    """
    Generator for creating and evaluating trading strategy splits.

    This class encapsulates the logic for preparing splits and tensors for
    Option Alpha trading analysis. It aligns trade and filter data, calculates
    reward-per-risk metrics, identifies potential split points in the data,
    and converts everything to PyTorch tensors for further analysis.

    The class separates hyperparameters (set at initialization) from data
    (provided at generation time), making it easier to reuse configurations
    ...
    left_only_filters : list[str], optional
        List of filter column names that should only generate left splits.
        Default is an empty list.
    right_only_filters : list[str], optional
    ...
    max_splits_per_filter : int | None, optional
        ...
        selected to distribute samples as evenly as possible across buckets.
    max_depth : int, optional
        ...
        with logical AND up to the specified depth.
    ...
    filter_granularities : dict[str, float] | None, optional
        Dictionary mapping filter names to granularity values. Default is None.
    ...
    ...     min_samples=10,
    ... )
    >>> X, y, splits = generator.generate(trades_df, filters_df, verbose="best")
    """

    def __init__(
        self,
        spread_width: int,
        left_only_filters: list[str] | None = None,
        right_only_filters: list[str] | None = None,
        ...
        self.left_only_filters = (
            left_only_filters if left_only_filters is not None else []
        )
        self.right_only_filters = (
            right_only_filters if right_only_filters is not None else []
        )
        self.device = device if device is not None else torch.device("cpu")
        self.dtype = dtype if dtype is not None else torch.float32
        self.max_splits_per_filter = max_splits_per_filter
        self.max_depth = max_depth
        self.min_samples = min_samples
        self.score_func = score_func
        self.keep_best_n = keep_best_n
        self.filter_eval_folds = filter_eval_folds
        self.filter_eval_repeats = filter_eval_repeats
        ...
        if filter_granularities is None:
            self.filter_granularities = default_granularities
        else:
            self.filter_granularities = {
                **default_granularities,
    ...
    def _create_splits_for_filter(
        ...
        list[Split]
            List of Split objects for this filter
        """
        return _create_splits_for_filter(
            col_idx,
            col_name,
            filters_df,
            left_only_filters,
    ...
    def generate(
        ...
        """
        Generate splits and tensors for Option Alpha trading analysis.
        ...
        spread_width : int | None, optional
            ...
            from the constructor. Provided for backwards compatibility.
        verbose : str, optional
            ...
            - "all": Print all splits in all_splits list at the end of each depth.
                     If score_func is not None, splits are printed in descending
                     score order.
        ...
        - Left splits include values <= threshold, right splits include values >=
          threshold.
        - At depth 2, child splits are created by combining depth 1 splits with each
          other (avoiding duplicates using j > i).
        ...
        if verbose == "best" and self.score_func is None:
            raise ValueError("verbose='best' requires score_func to be not None")

        # Use spread_width from parameter if provided, otherwise use from constructor
        actual_spread_width = (
            spread_width if spread_width is not None else self.spread_width
        )

        # Align filters_df with trades_df
        filters_df = _align_filters_with_trades(filters_df, trades_df)

        # Add computed columns and update left_only_filters and right_only_filters
        left_only_filters_updated: list[str]
        right_only_filters_updated: list[str]
        filters_df, left_only_filters_updated, right_only_filters_updated = (
        ...
        for col_idx, col_name in enumerate(filter_names):
            filter_splits = self._create_splits_for_filter(
                ...
                col_name,
                filters_df,
                left_only_filters_updated,
                right_only_filters_updated,
            )
        ...
        if self.score_func is not None:
            ...
            if (
                self.min_score_improvement is not None
                and len(filter_score_improvements) > 0
            ):
                filtered_splits = []
                for split in depth_1_splits:
                ...
                if removed_count > 0:
                    print(
                        ...
                        f"< {self.min_score_improvement:.6f}"
                    )
        ...
        if self.keep_best_n is not None:
            all_splits = _keep_top_n_splits(depth_1_splits, self.keep_best_n)
        else:
        ...
        if verbose == "best":
            if len(all_splits) > 0:
                ...
                _print_splits_for_depth(1, [best_split], self.score_func)
        elif verbose == "all":
            _print_splits_for_depth(1, all_splits, self.score_func)
        ...
        if self.max_depth > 1:
            ...
            for depth in range(2, self.max_depth + 1):
                ...
                if self.keep_best_n is not None and self.score_func is not None:
                    ...
                    batch_size = 2 * self.keep_best_n

                    # Loop to ensure we get enough valid candidates
                    while len(
                        candidate_splits
                    ) < self.keep_best_n and candidate_start_idx < len(new_splits):
                    ...
                    all_splits = _keep_top_n_splits(all_splits, self.keep_best_n)
                    ...
                    if self.keep_best_n is not None:
                        ...
                        # Update previous_depth_splits to contain only kept splits from new_splits
                        all_splits_set = set(id(s) for s in all_splits)
                        previous_depth_splits = [
                ...
                if verbose == "best":
                    if len(all_splits) > 0:
                        ...
                        _print_splits_for_depth(depth, [best_split], self.score_func)
                elif verbose == "all":
                    _print_splits_for_depth(depth, all_splits, self.score_func)
...
def prepare_splits(
    ...
    .. deprecated::
        Use :class:`SplitGenerator` instead. This function is kept for backwards
        compatibility but will be removed in a future version.
    """
    generator = SplitGenerator(
        spread_width=spread_width,
        left_only_filters=left_only_filters,
        right_only_filters=right_only_filters,
        device=device,
        dtype=dtype,
        max_splits_per_filter=max_splits_per_filter,
        max_depth=max_depth,
        min_samples=min_samples,
        score_func=score_func,
        keep_best_n=keep_best_n,
        filter_granularities=filter_granularities,
        filter_eval_folds=filter_eval_folds,
        filter_eval_repeats=filter_eval_repeats,
