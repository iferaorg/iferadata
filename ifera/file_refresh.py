...
def download_file(source: str, type: str, interval: str, symbol: str) -> None:
    """
    ...
    file_path = make_path(source, type, interval, symbol, zipfile=True)
    file_url = make_url("file", source, type, interval, symbol, zipfile=True)
    fm = FileManager()

    if not fm.is_up_to_date(file_url):
        bucket = settings.S3_BUCKET
        download_s3_file(bucket, path, str(file_path))


def process_raw_file(type: str, interval: str, symbol: str) -> None:
    """
    ...
    file_path = make_path("processed", type, interval, symbol, zipfile=True)
    file_url = make_url("file", "processed", type, interval, symbol, zipfile=True)
    fm = FileManager()
    cm = ConfigManager()
    raw_file_path = make_path("raw", type, interval, symbol, zipfile=True)
    ...
    if not fm.is_up_to_date(file_url):
        ...
        bucket = settings.S3_BUCKET
        key = make_s3_key("processed", instrument, zipfile=True)
