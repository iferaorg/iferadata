...
def compress_tensor(t, mask):
    ...
    flat_compressed[row_idx[flat_mask], flat_valid_indices[flat_mask]] = flat_t[flat_mask]
...
def decompress_tensor(compressed, mask):
    ...
    flat_decompressed[flat_mask] = flat_compressed[row_idx[flat_mask], flat_valid_indices[flat_mask]]
...
def masked_sma(t: MaskedTensor, window: int) -> MaskedTensor:
    ...
    Returns:
        MaskedTensor: Masked tensor containing the SMA values with the same shape and mask as the input.
    """
    data = t.get_data()
    mask = t.get_mask()
...
def masked_ema(t: MaskedTensor, alpha: float, chunk_size: Optional[int] = None) -> MaskedTensor:
    """
    ...
    Returns:
        MaskedTensor: Masked tensor containing the EMA values with the same shape and mask as the input.
    """
    data = t.get_data()
    mask = t.get_mask()
...
def masked_rtr(t: MaskedTensor) -> MaskedTensor:
    ...
    cdata = compress_tensor(data_cn, mask_cn).nan_to_num(nan=1.0)   # Avoid division by zero
...
def masked_artr(t: MaskedTensor, alpha: float, acrossday: bool = False, chunk_size: Optional[int] = None) -> MaskedTensor:
    """
    ...
    Args:
        ...
        chunk_size (Optional[int], optional): If provided, calculate the EMA in chunks of this size. Defaults to None.
