...
batch_size = 1#idata.data.shape[0]-250
...
date_idx = torch.arange(3084, 3084+1, dtype=torch.int32, device=idata.device)
...
#    prev_stop_loss = stop_loss.clone()
    action, stop_loss = tradingPolicy(date_idx, time_idx, position, prev_stop_loss, execution_price)
    done = done | ((date_idx == max_date_idx) & (time_idx >= max_time_idx - 1))
    action = torch.where(done, -position, 0)
    time_idx = torch.where(done, max_time_idx - 1, time_idx)
    date_idx, time_idx = idata.get_next_indices(date_idx, time_idx)
    profit, position, execution_price, _ = ms.calculate_step(date_idx, time_idx, position, action, stop_loss)
#    position = position.clone()
    total_profit += profit
    if position.sum() == 0:# or dt.datetime.now() - timer > dt.timedelta(minutes=10):
        break
