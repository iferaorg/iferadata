...
def test_compress_decompress():
    ...
    torch.testing.assert_close(decompressed, expected, rtol=1e-4, atol=1e-4, equal_nan=True)
...
def test_ohlcv_to_masked():
    ...
    torch.testing.assert_close(masked.get_mask().float(), expected_mask.float(), rtol=1e-4, atol=1e-4)
...
def test_masked_artr(ohlcv_single_date_masked, ohlcv_single_date):
    ...
    torch.testing.assert_close(result_masked_flat.get_data(), expected, rtol=1e-4, atol=1e-4)
