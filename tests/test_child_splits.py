...
def test_exclusion_mask_updated_for_child_splits():
    """Test that child splits are generated correctly at depth 2."""
    trades_df = pd.DataFrame(
        {"risk": [100.0] * 4, "profit": [50.0] * 4},
        index=pd.DatetimeIndex(
            ["2022-01-10", "2022-01-11", "2022-01-12", "2022-01-13"], name="date"
    ...
    depth1_count = sum(1 for s in splits if len(s.parents) == 0)
    child_count = len(splits) - depth1_count

    assert depth1_count > 0, "Should have depth 1 splits"
    assert child_count > 0, "Should have child splits at depth 2"
...
def test_child_splits_respect_exclusion_mask():
    ...
    depth1_splits = [s for s in splits if len(s.parents) == 0]
    child_splits = [s for s in splits if len(s.parents) > 0]

    # Verify that all child splits have non-empty masks (at least min_samples)
    # This indirectly shows they weren't created from exclusive parents
    for child in child_splits:
        assert child.mask.sum().item() >= 1, "Child splits should have at least 1 sample"
        
        # Verify that child mask is actually the AND of its parents
        for parent_i, parent_j in child.parents:
            expected_mask = depth1_splits[parent_i].mask & depth1_splits[parent_j].mask
            assert torch.equal(child.mask, expected_mask), \
                "Child mask should be AND of parent masks"
